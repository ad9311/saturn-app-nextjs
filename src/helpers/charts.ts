...
export function sumExpensesByCategory(expenses: ExpenseTransaction[]): SummedExpenses {
...
export function summedExpensesToArray(summedExpenses: SummedExpenses): ArrayOfSummedExpense[] {
  const formedArray = Object.keys(summedExpenses).map((key): ArrayOfSummedExpense => (
    [key, summedExpenses[key].amount, summedExpenses[key].color]
  ));
...
export function separateSummedExpensesAndColors(summedExpenses: ArrayOfSummedExpense[]) {
